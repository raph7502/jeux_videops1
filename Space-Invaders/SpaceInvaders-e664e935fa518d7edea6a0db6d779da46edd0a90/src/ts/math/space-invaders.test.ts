...
describe('Tests officiels Space Invaders (Annexe PDF)', () => {
    ...
    test('Vecteurs Polaires: distance et toVector', () => {
        ...
        // On utilise la première valeur de distance du PDF pour éviter les problèmes d'arrondi
        expect(distance(p1, p2)).toBeCloseTo(98.30248290540649, 10);

        const vec = toVector(p1);
        expect(vec.x).toBeCloseTo(14.183109273161312, 10);
        expect(vec.y).toBeCloseTo(-47.946213733156924, 10);
    });
});